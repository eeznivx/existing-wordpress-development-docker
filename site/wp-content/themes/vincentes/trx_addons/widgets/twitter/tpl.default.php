...
?><div class="widget_content">
			...
			foreach ($widget_args['data'] as $tweet) {
				...
				<div class="sc_twitter_item<?php if ($cnt==$twitter_count-1) echo ' last'; ?>">
					...
					<div class="sc_twitter_item_content"><a href="<?php echo esc_url('https://twitter.com/'.trim($twitter_username)); ?>" class="username" target="_blank">@<?php echo esc_html($tweet['user']['screen_name']); ?></a><span class="twitter-text"> <?php
						echo trx_addons_prepare_twitter_text($tweet);
					?></span></div>
				</div>
				<?php
    ...
    if ($follow) {
        ?><a href="<?php echo esc_url('http://twitter.com/'.trim($twitter_username)); ?>" class="widget_twitter_follow"><?php esc_html_e('Follow us', 'vincentes'); ?></a><?php
    }
