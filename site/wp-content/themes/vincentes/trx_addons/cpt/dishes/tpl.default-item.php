...
<div class="sc_dishes_item with_image<?php
		...
		?><div class="sc_dishes_item_image"><?php
			trx_addons_get_template_part('templates/tpl.featured.php',
										...
										apply_filters('trx_addons_filter_args_featured', array(
														...
														'hover' => 'zoomin',
														'thumb_size' => apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size($args['columns'] > 2 ? 'full' : 'big'), 'dishes-default')
														),
														'dishes-default'
													)
										);
			...
			if (trim($meta['spicy']) != '') {
				...
				?><span class="dishes_page_spicy dishes_page_spicy_<?php echo esc_html($meta['spicy']); ?>">
					<span class="dishes_page_spicy_label"><?php esc_html_e('Spicy Level:', 'vincentes'); ?></span>
	...
	<div class="sc_dishes_item_info">
		<div class="sc_dishes_item_header">
			<h4 class="sc_dishes_item_title"><a href="<?php echo esc_url($link); ?>"><?php the_title();

                    // Price
                    if (trim($meta['price']) != '') {
                        ?> - <span class="sc_dishes_price"><?php echo esc_html($meta['price']); ?></span><?php
                    }
                    ?></a></h4>
		</div>
