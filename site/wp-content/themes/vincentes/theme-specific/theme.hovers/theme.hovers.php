...
if (!function_exists('vincentes_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'vincentes_hovers_theme_setup3', 3 );
	function vincentes_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		vincentes_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'vincentes'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'vincentes') ),
				"std" => 'slide_left',
				"options" => array(
					'default'		=> esc_html__('Fade',				'vincentes'),
					'slide_left'	=> esc_html__('Slide from Left',	'vincentes'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'vincentes'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'vincentes') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'vincentes'),
					'icon'	=> esc_html__('Icon',	'vincentes'),
					'icons'	=> esc_html__('Icons',	'vincentes'),
					'zoom'	=> esc_html__('Zoom',	'vincentes'),
					'fade'	=> esc_html__('Fade',	'vincentes'),
					'slide'	=> esc_html__('Slide',	'vincentes'),
					'pull'	=> esc_html__('Pull',	'vincentes'),
					'border'=> esc_html__('Border',	'vincentes')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('vincentes_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'vincentes_hovers_theme_setup9', 9 );
	function vincentes_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'vincentes_hovers_frontend_scripts', 1010 );
		add_filter( 'vincentes_filter_localize_script','vincentes_hovers_localize_script' );
		add_filter( 'vincentes_filter_merge_scripts',	'vincentes_hovers_merge_scripts' );
		add_filter( 'vincentes_filter_merge_styles',	'vincentes_hovers_merge_styles' );
		add_filter( 'vincentes_filter_get_css', 		'vincentes_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'vincentes_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'vincentes_hovers_frontend_scripts', 1010 );
	function vincentes_hovers_frontend_scripts() {
		if ( vincentes_is_on(vincentes_get_theme_option('debug_mode')) && vincentes_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'vincentes-hovers', vincentes_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( vincentes_is_on(vincentes_get_theme_option('debug_mode')) && vincentes_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'vincentes-hovers',  vincentes_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('vincentes_hovers_merge_scripts')) {
	//Handler of the add_filter( 'vincentes_filter_merge_scripts', 'vincentes_hovers_merge_scripts' );
	function vincentes_hovers_merge_scripts($list) {
...
if (!function_exists('vincentes_hovers_merge_styles')) {
	//Handler of the add_filter( 'vincentes_filter_merge_styles', 'vincentes_hovers_merge_styles' );
	function vincentes_hovers_merge_styles($list) {
...
if (!function_exists('vincentes_hovers_localize_script')) {
	//Handler of the add_filter( 'vincentes_filter_localize_script','vincentes_hovers_localize_script' );
	function vincentes_hovers_localize_script($arr) {
		$arr['button_hover'] = vincentes_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('vincentes_hovers_add_icons') ) {
	function vincentes_hovers_add_icons($hover, $args=array()) {
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Add to cart', 'vincentes') : '')
										. '</a>',
										$product );
				}
				?>
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php esc_url(the_permalink()); ?>" aria-hidden="true" class="icon-chain"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'vincentes_hovers_get_css' ) ) {
	//Handler of the add_filter( 'vincentes_filter_get_css', 'vincentes_hovers_get_css', 10, 4 );
	function vincentes_hovers_get_css($css, $colors, $fonts, $scheme='') {
...
.sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['text_link']} 50%, {$colors['text_dark']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_link']} !important; }
.sc_button_hover_slide_right {  background: linear-gradient(to left,	{$colors['text_link']} 50%, {$colors['text_dark']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['text_link']} !important; }
.sc_button_hover_slide_top {	background: linear-gradient(to bottom,	{$colors['text_link']} 50%, {$colors['text_dark']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['text_link']} !important; }
.sc_button_hover_slide_bottom {	background: linear-gradient(to top,		{$colors['text_link']} 50%, {$colors['text_dark']} 50%) no-repeat scroll right top / 100% 210% {$colors['text_link']} !important; }
...
.sc_button_hover_style_alter.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_dark']} 50%, {$colors['text_link']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['alter_link']} !important; }
.sc_button_hover_style_alter.sc_button_hover_slide_right {		background: linear-gradient(to left,	{$colors['text_dark']} 50%, {$colors['text_link']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['alter_link']} !important; }
.sc_button_hover_style_alter.sc_button_hover_slide_top {		background: linear-gradient(to bottom,	{$colors['text_dark']} 50%, {$colors['text_link']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['alter_link']} !important; }
.sc_button_hover_style_alter.sc_button_hover_slide_bottom {		background: linear-gradient(to top,		{$colors['text_dark']} 50%, {$colors['text_link']} 50%) no-repeat scroll right top / 100% 210% {$colors['alter_link']} !important; }
