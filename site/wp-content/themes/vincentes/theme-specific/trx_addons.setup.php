...
------------------------------------------------------------------------------- */
if (!function_exists('vincentes_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'vincentes_trx_addons_theme_specific_components');
	function vincentes_trx_addons_theme_specific_components($enable=false) {
		return false;
	}
}

if (!function_exists('vincentes_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'vincentes_trx_addons_theme_specific_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'vincentes_trx_addons_theme_specific_setup1', 8 );
	function vincentes_trx_addons_theme_specific_setup1() {
		if (vincentes_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',				'vincentes_trx_addons_cv_enable');
			add_filter( 'trx_addons_cpt_list',				'vincentes_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',				'vincentes_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',			'vincentes_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'vincentes_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'vincentes_trx_addons_cv_enable');
	function vincentes_trx_addons_cv_enable($enable=false) {
...
// CPT
if ( !function_exists( 'vincentes_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'vincentes_trx_addons_cpt_list');
	function vincentes_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'vincentes_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'vincentes_trx_addons_sc_list');
	function vincentes_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'vincentes_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'vincentes_trx_addons_widgets_list');
	function vincentes_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'vincentes_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'vincentes_trx_addons_menu_cache');
	function vincentes_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('vincentes_trx_addons_localize_script')) {
	...
	function vincentes_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = vincentes_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'vincentes_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'vincentes_trx_addons_sc_type', 10, 2);
	function vincentes_trx_addons_sc_type($list, $sc) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list

        if (in_array($sc, array('trx_sc_button'))) {
            $list[ esc_html__('Alternative', 'vincentes') ] = 'alter';
        }

        if (in_array($sc, array('trx_sc_events'))) {
            $list[ esc_html__('Full style', 'vincentes') ] = 'full';
        }
...
if ( !function_exists( 'vincentes_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'vincentes_trx_addons_sc_atts', 10, 2);
	function vincentes_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'vincentes_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'vincentes_trx_addons_sc_map', 10, 2);
	function vincentes_trx_addons_sc_map($params, $sc) {
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'vincentes'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'vincentes') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array(
				esc_html__('Default', 'vincentes') => 'default',
				esc_html__('Link 2', 'vincentes') => 'link2',
				esc_html__('Link 3', 'vincentes') => 'link3',
				esc_html__('Dark', 'vincentes') => 'dark'
			),
			'type' => 'dropdown'
		);
		if (in_array($sc, array('trx_sc_button'))) {
...
if ( !function_exists( 'vincentes_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'vincentes_trx_addons_sc_output', 10, 4);
	function vincentes_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !vincentes_is_inherit($atts['color_style']))
...
if ( !function_exists( 'vincentes_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'vincentes_trx_addons_sc_item_title_tag');
	function vincentes_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'vincentes_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'vincentes_trx_addons_sc_item_button_args', 10, 3);
	function vincentes_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'vincentes_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'vincentes_trx_addons_slider_title', 10, 2 );
	function vincentes_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'vincentes_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'vincentes_trx_addons_sc_googlemap_styles');
	function vincentes_trx_addons_sc_googlemap_styles($list) {
		$list[esc_html__('Dark', 'vincentes')] = 'dark';
        $list[esc_html__('Grey', 'vincentes')] = 'grey';
		return $list;
	}
}
...
if ( !function_exists( 'vincentes_trx_addons_editor_init' ) ) {
	if (is_admin()) add_filter( 'tiny_mce_before_init', 'vincentes_trx_addons_editor_init', 11);
	function vincentes_trx_addons_editor_init($opt) {
		if (vincentes_exists_trx_addons()) {
			...
			if (!empty($opt['style_formats'])) {
				...
				if (is_array($style_formats) && count($style_formats)>0 ) {
					foreach ($style_formats as $k=>$v) {
						if ( $v['title'] == esc_html__('List styles', 'vincentes') ) {
							$style_formats[$k]['items'][] = array(
										'title' => esc_html__('Leaf', 'vincentes'),
										'selector' => 'ul',
										'classes' => 'trx_addons_list trx_addons_list_leaf'
									);
						}
                        if ( $v['title'] == esc_html__('Inline', 'vincentes') ) {
                            $style_formats[$k]['items'][] = array(
                                'title' => esc_html__('Accent hovered', 'vincentes'),
                                'inline' => 'span',
                        ...
                        if ( $v['title'] == esc_html__('Inline', 'vincentes') ) {
                            $style_formats[$k]['items'][] = array(
                                'title' => esc_html__('Accent hovered(bg)', 'vincentes'),
                                'inline' => 'span',
...
if ( !function_exists( 'vincentes_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'vincentes_filter_allow_override_header_image', 'vincentes_trx_addons_allow_override_header_image' );
	function vincentes_trx_addons_allow_override_header_image($allow) {
		return vincentes_is_team_page() || vincentes_is_portfolio_page() ? false : $allow;
	}
}

// Hide sidebar on the team and portfolio pages
if ( !function_exists( 'vincentes_trx_addons_sidebar_present' ) ) {
	add_filter( 'vincentes_filter_sidebar_present', 'vincentes_trx_addons_sidebar_present' );
	function vincentes_trx_addons_sidebar_present($present) {
		return !is_single() && (vincentes_is_team_page() || vincentes_is_portfolio_page()) ? false : $present;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'vincentes_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'vincentes_trx_addons_thumb_size', 10, 2);
	function vincentes_trx_addons_thumb_size($thumb_size='', $type='') {
		if ($type == 'team-default')
			$thumb_size = vincentes_get_thumb_size('big');
		return $thumb_size;
	}
}
...
if (!function_exists('vincentes_trx_addons_override_options_fields')) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'vincentes_trx_addons_override_options_fields', 10, 2);
	function vincentes_trx_addons_override_options_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'vincentes'),
				"desc" => wp_kses_data( __("Team member's email", 'vincentes') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
