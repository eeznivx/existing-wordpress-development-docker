...
if ( !function_exists('vincentes_need_page_title') ) {
	function vincentes_need_page_title() {
		return !is_front_page() && apply_filters('vincentes_filter_need_page_title', true);
	}
}
...
if ( !function_exists('vincentes_show_layout') ) {
	function vincentes_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('vincentes_get_logo_image') ) {
	function vincentes_get_logo_image($type='') {
		$logo_image = '';
		if (vincentes_get_retina_multiplier(2) > 1)
			$logo_image = vincentes_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = vincentes_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}

// Return header video (if set)
if ( !function_exists('vincentes_get_header_video') ) {
	function vincentes_get_header_video() {
		$video = '';
		if (apply_filters('vincentes_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (vincentes_check_theme_option('header_video')) {
				$video = vincentes_get_theme_option('header_video');
...
if ( !function_exists('vincentes_show_post_meta') ) {
	function vincentes_show_post_meta($args=array()) {
		...
		?><div class="post_meta"><?php
			...
			foreach ($components as $comp) {
				...
				// Post date
					...
					$dt = apply_filters('vincentes_filter_get_post_date', vincentes_get_date());
					if (!empty($dt)) {
						?>
				...
				// Post author
					...
					if ($author_id > 0) {
						...
						?><span class="post_meta_item post_author"><?php esc_html_e('Posted by', 'vincentes');?>
						<a class="post_author" rel="author" href="<?php echo esc_url($author_link); ?>">
				 ...
				 // Post counters
                    ...
                    if ( ($output = vincentes_get_post_counters($args['counters'])) != '')
                        vincentes_show_layout($output);
                    else {
                        if (!is_singular() || have_comments() || comments_open()) {
                            ...
                            <a href="<?php echo esc_url(get_comments_link()); ?>" class="post_meta_item post_counters_item post_counters_comments trx_addons_icon-comment"><?php
                                ...
                                ?><span class="post_counters_label"><?php
                                    echo esc_html(_n('Comment', 'Comments', $post_comments, 'vincentes'));
                                ?></span>
                            </a>
                            <?php
                        }
                ...
                // Socials share
					...
					vincentes_show_share_links(array(
							'type' => 'drop',
							'caption' => esc_html__('Share', 'vincentes'),
				...
				// Edit page link
					...
					edit_post_link( esc_html__( 'Edit', 'vincentes' ), '<span class="post_meta_item post_edit">', '</span>' );
				}
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('vincentes_show_post_featured') ) {
	function vincentes_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => vincentes_get_theme_option('image_hover'),	// Hover effect
		...
		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : vincentes_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? vincentes_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = vincentes_get_post_image();
				if (!empty($image)) 
					$image = vincentes_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . vincentes_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"<?php if ($args['seo']) echo ' itemscope itemprop="image" itemtype="http://schema.org/ImageObject"'; ?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$vincentes_attr = vincentes_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						vincentes_hovers_add_icons($args['hover']);
					}
					...
					if (($output=vincentes_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						vincentes_show_layout($output);
					...
					if (!empty($image)) {
						...
						vincentes_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				if ( ! $args['thumb_only'] && ( in_array( $post_format, array( 'video', 'audio' ) ) ) ) {
					$post_content = vincentes_get_post_content();
					$post_content_parsed = $post_content;

					if ( 'video' == $post_format ) {
						$video = vincentes_get_post_video( $post_content, false );
						if ( empty( $video ) ) {
							$video = vincentes_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							// Only get video from the content if a playlist isn't present.
							$post_content_parsed = vincentes_filter_post_content( $post_content );
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = vincentes_array_get_first( $videos, false );
								}
							}
						}
						if ( ! empty( $video ) ) {
							if ( $has_thumb ) {
								$video = vincentes_make_video_autoplay( $video );
								?>
						...
						$audio = vincentes_get_post_audio( $post_content, false );
						if ( empty( $audio ) ) {
							$audio = vincentes_get_post_iframe( $post_content, false );
						}
						...
						if ( empty( $audio ) ) {
							...
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $audios ) && is_array( $audios ) ) {
									$audio = vincentes_array_get_first( $audios, false );
								}
							}
						}
						if ( ! empty( $audio ) ) {
							...
							">
								...
								if ( strpos( $audio, '<audio' ) !== false ) {
									$media_author = vincentes_get_tag_attrib( $audio, '<audio>', 'data-author' );
									$media_title  = vincentes_get_tag_attrib( $audio, '<audio>', 'data-caption' );
								}
								if ( empty( $media_author) &&  empty( $media_title) ) {
									$media = urldecode( vincentes_get_tag_attrib( $post_content, '[trx_widget_audio]', 'media' ) );
									if ( ! empty( $media ) ) {
										...
										// Parse tag params
										$media_author = strip_tags( vincentes_get_tag( $post_content_parsed, '<h6 class="audio_author">', '</h6>' ) );
										$media_title  = strip_tags( vincentes_get_tag( $post_content_parsed, '<h5 class="audio_caption">', '</h5>' ) );

									}
								}
								if ( ! empty( $media_author ) ) {
									?>
									<div class="post_audio_author"><?php vincentes_show_layout( $media_author ); ?></div>
									<?php
								}
								if ( ! empty( $media_title ) ) {
									?>
									<h5 class="post_audio_title"><?php vincentes_show_layout( $media_title ); ?></h5>
									<?php
								}
								// Display audio
								vincentes_show_layout( $audio );
								?>
							</div>
							<?php
						}
				...
				vincentes_show_layout($post_info);
				?></div><?php
			} else {
				// Put optional info block over the thumb
				vincentes_show_layout($post_info);
...
if ( !function_exists('vincentes_get_no_image') ) {
	function vincentes_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = vincentes_get_theme_option( 'no_image' );
			if (empty($img)) $img = vincentes_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('vincentes_add_bg_in_post_nav') ) {
	function vincentes_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = vincentes_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), vincentes_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), vincentes_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'vincentes-main', $css );
	}
}

// Show related posts
if ( !function_exists('vincentes_show_related_posts') ) {
	function vincentes_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'vincentes');
				?></h3>
...
if ( !function_exists('vincentes_show_portfolio_posts') ) {
	function vincentes_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? vincentes_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = vincentes_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = vincentes_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the vincentes_ajax_get_posts action
if ( !function_exists( 'vincentes_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_vincentes_ajax_get_posts',			'vincentes_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_vincentes_ajax_get_posts',	'vincentes_ajax_get_posts_callback');
	function vincentes_ajax_get_posts_callback() {
		...
		if ($id > 0) {
			...
			vincentes_storage_set('blog_mode', 'blog');
			vincentes_storage_set('options_meta', get_post_meta($id, 'vincentes_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => vincentes_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'vincentes');
		}
...
// Show pagination
if ( !function_exists('vincentes_show_pagination') ) {
	function vincentes_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = vincentes_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'vincentes' ),
				'next_text' => esc_html__( '>', 'vincentes' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'vincentes' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'vincentes'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'vincentes') : esc_html__('Newest posts', 'vincentes') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'vincentes') : esc_html__('Older posts', 'vincentes'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'vincentes_single_comments_field' ) ) {
    function vincentes_single_comments_field( $args ) {
